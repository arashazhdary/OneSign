...
export default function TenantServiceAccountsPage() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess(t('tenant.serviceAccounts.messages.created'));
      setShowModal(false);
      fetchServiceAccounts();
      setForm({ name: '', description: '' });
      setTimeout(() => setSuccess(''), 3000);
    } catch (err) {
      setError(t('tenant.serviceAccounts.messages.failedToCreate'));
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 dark:from-slate-900 dark:via-slate-900 dark:to-slate-800">
      <Helmet>
        <title>{t('tenant.serviceAccounts.pageTitle')}</title>
      </Helmet>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        >
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-3xl font-bold text-gray-900 dark:text-white">{t('tenant.serviceAccounts.title')}</h1>
              <p className="text-gray-500 dark:text-gray-400 mt-1">{t('tenant.serviceAccounts.subtitle')}</p>
            </div>
          </div>
          <motion.button
          ...
          >
            ...
            <span>{t('tenant.serviceAccounts.buttons.create')}</span>
          </motion.button>
        </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <StatCard
            title={t('tenant.serviceAccounts.stats.totalAccounts')}
          ...
          />
          <StatCard
            title={t('tenant.serviceAccounts.stats.active')}
          ...
          />
          <StatCard
            title={t('tenant.serviceAccounts.stats.inactive')}
            value={serviceAccounts.filter(sa => !sa.isEnabled).length}
        ...
        <div className="space-y-4">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-white">{t('tenant.serviceAccounts.sections.allAccounts')}</h2>
          <AnimatePresence>
            {serviceAccounts.length === 0 ? (
              ...
              >
                ...
                <p className="text-gray-500 dark:text-gray-400">{t('tenant.serviceAccounts.empty.noAccounts')}</p>
                <p className="text-sm text-gray-400 dark:text-gray-500 mt-1">{t('tenant.serviceAccounts.empty.createHint')}</p>
              </motion.div>
            ) : (
                ...
                >
                  <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                      ...
                      <div>
                        <div className="flex items-center space-x-3">
                          ...
                          <span className={`px-3 py-1 text-xs font-medium rounded-full ${
                            ...
                            {account.isEnabled ? t('common.active') : t('common.inactive')}
                          </span>
                        </div>
                        ...
                        <div className="mt-3 flex flex-wrap items-center gap-4 text-sm">
                          ...
                          <div className="flex items-center space-x-2 text-gray-500 dark:text-gray-400">
                            ...
                            <span>{t('tenant.serviceAccounts.labels.created')} {new Date(account.createdAt).toLocaleDateString()}</span>
                          </div>
                          {account.lastUsed && (
                            <div className="flex items-center space-x-2 text-gray-500 dark:text-gray-400">
                              <RefreshCw className="w-4 h-4" />
                              <span>{t('tenant.serviceAccounts.labels.lastUsed')} {new Date(account.lastUsed).toLocaleString()}</span>
                            </div>
                          )}
                        </div>
                      </div>
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      >
                        <RefreshCw className="w-4 h-4" />
                        <span>{t('tenant.serviceAccounts.buttons.regenerateSecret')}</span>
                      </motion.button>
                      <motion.button
                        whileHover={{ scale: 1.05 }}
                        whileTap={{ scale: 0.95 }}
        ...
        >
          <div className="flex items-start space-x-4">
            ...
            <div>
              <h3 className="font-semibold text-blue-900 dark:text-blue-100">{t('tenant.serviceAccounts.security.title')}</h3>
              <p className="text-sm text-blue-700 dark:text-blue-300 mt-1">
                {t('tenant.serviceAccounts.security.description')}
              </p>
            </div>
          </div>
        </motion.div>
      ...
      {/* Create Modal */}
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} title={t('tenant.serviceAccounts.modal.createTitle')}>
        <form onSubmit={handleCreate} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{t('tenant.serviceAccounts.form.name')}</label>
            <input
              type="text"
              value={form.name}
              onChange={(e) => setForm({ ...form, name: e.target.value })}
              placeholder={t('tenant.serviceAccounts.form.namePlaceholder')}
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{t('tenant.serviceAccounts.form.description')}</label>
            <textarea
              value={form.description}
              onChange={(e) => setForm({ ...form, description: e.target.value })}
              placeholder={t('tenant.serviceAccounts.form.descriptionPlaceholder')}
              className="w-full px-4 py-2 border border-gray-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              rows={3}
            />
          </div>
          <div className="flex justify-end space-x-3 pt-4">
            ...
            >
              {t('tenant.serviceAccounts.buttons.createAccount')}
            </motion.button>
          </div>
        </form>
      </Modal>
      ...
      {/* Secret Modal */}
      <Modal isOpen={showSecretModal} onClose={() => setShowSecretModal(false)} title={t('tenant.serviceAccounts.modal.secretTitle')}>
        <div className="space-y-4">
          <div className="p-4 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-700 rounded-lg flex items-start space-x-3">
            ...
            <p className="text-sm text-yellow-700 dark:text-yellow-300">
              {t('tenant.serviceAccounts.modal.secretWarning')}
            </p>
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{t('tenant.serviceAccounts.modal.secretLabel')}</label>
